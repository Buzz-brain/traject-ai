...
const STATUS_CONFIG: Record<RecordingStatus, { label: string; color: string; pulse: boolean }> = {
  ...
  recording: { label: 'Tracking', color: 'text-purple-400', pulse: true },
  ...
  return (
    <header className="flex items-center justify-between px-4 py-3">
      <div className="flex items-center gap-2">
        <div className="w-8 h-8 bg-purple-500 rounded-xl flex items-center justify-center shadow-lg shadow-purple-500/30">
          <Zap size={16} className="text-white" />
        </div>
        <div>
          <h1 className="text-sm font-bold text-slate-800 dark:text-white leading-none">TrajectAI</h1>
          <p className="text-xs text-slate-500 dark:text-slate-400 leading-none mt-0.5">Hybrid GPS + AI</p>
        </div>
      </div>
